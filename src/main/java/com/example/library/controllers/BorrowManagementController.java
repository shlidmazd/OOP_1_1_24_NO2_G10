...
//        });
    ...
    public void onClickBorrow(ActionEvent actionEvent) {
        ...
        if (isNull(cbReaderId.getValue(), cbBookId.getValue(), dpReturnDate.getValue())) {
            Alert.showAlert(javafx.scene.control.Alert.AlertType.ERROR, "Lỗi", null, "Vui lòng chọn độc giả, sách và ngày trả");
            return;
        }

        if (borrowService.getTotalBorrowByReaderId(cbReaderId.getValue()) >= 3) {
            Alert.showAlert(javafx.scene.control.Alert.AlertType.ERROR, "Lỗi", null, "Độc giả đã mượn quá số lượng cho phép (<=3)");
            return;
        }

        if (!bookService.isQuantityEnough(cbBookId.getValue())) {
            Alert.showAlert(javafx.scene.control.Alert.AlertType.ERROR, "Lỗi", null, "Số lượng sách không đủ");
            return;
        }
        ...
        Alert.showAlert(javafx.scene.control.Alert.AlertType.INFORMATION, "Thông báo", null, "Mượn sách thành công");
        tbBorrows.setItems(borrowService.getAllBookBorrowed());
    }
